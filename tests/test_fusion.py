...
def test_мёртвая_зона_не_плодит_вторую_станцию(конфиг):
    """Одна станция не имеет права стать двумя.

    Между assoc_radius и min_separation наблюдение слишком далеко, чтобы уточнять
    координату, но ближе, чем вторая станция может стоять физически (замер площадки:
    25 см между краями плюс 21 см самой станции = 46 см между центрами). Раньше такое
    наблюдение заводило новую запись — обрывок панели становился отдельной станцией,
    а это те же 8 баллов, что и пропущенная.
    """
    р = реестр(конфиг, assoc_radius=0.20, min_separation=0.40)
    р.observe(0.0, 0.0)
    р.observe(0.30, 0.0)               # мёртвая зона: 0.20 < 0.30 < 0.40
    assert len(р.stations(confirmed_only=False)) == 1, "станция раскололась надвое"
    assert р.prevented_splits == 1

    станция = р.stations(confirmed_only=False)[0]
    assert станция.x == pytest.approx(0.0), \
        "грубое наблюдение утащило координату: оно вправе подтверждать станцию, но не двигать её"

    р.observe(0.50, 0.0)               # дальше физического минимума — уже другая
    assert len(р.stations(confirmed_only=False)) == 2


def test_защита_от_раскола_выключается(конфиг):
    """min_separation ниже assoc_radius означает «защиты нет» — поведение как раньше."""
    р = реестр(конфиг, assoc_radius=0.20, min_separation=0.0)
    р.observe(0.0, 0.0)
    р.observe(0.30, 0.0)
    assert len(р.stations(confirmed_only=False)) == 2
    assert р.prevented_splits == 0
