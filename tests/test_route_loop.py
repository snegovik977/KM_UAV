...
def контурный_конфиг():
    ...
    прямоугольник от точки взлёта (см. tests/test_route_rect.py); контур остался
    запасным вариантом на случай, если площадку снова разложат вокруг «H»."""
...
def test_переключение_режима_стоит_одной_строки():
    """Контур достаётся из боевого конфига одной строкой, чем бы ни было умолчание.
    Автомат миссии режимы не различает, поэтому важно, что контур реально строится
    и после того, как основным стал прямоугольник."""
    cfg = config_module.load()
