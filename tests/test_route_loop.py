...
def контурный_конфиг():
    """Боевой конфиг переключён на контур. С 01.08.2026 умолчание в config.yaml —
    змейка (станции разбросаны по площади, см. tests/test_survey.py); контур остался
    режимом подзадачи 2.3.1, где оценивают сам облёт территории."""
...
def test_переключение_режима_стоит_одной_строки():
    """Оба режима достаются из одного и того же боевого конфига одной строкой:
    змейка — умолчание (станции разбросаны по площади), контур — для подзадачи 2.3.1.
    Автомат миссии их не различает, поэтому важно, что оба реально строятся."""
    cfg = config_module.load()
    assert plan_from_config(cfg).label == "змейка", "умолчание конфига — змейка"

    cfg["route"]["mode"] = "loop"
    контур = plan_from_config(cfg)
    assert контур.label == "облёт контура"
    assert len(контур.waypoints) == 5
