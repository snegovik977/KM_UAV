...
def test_габариты_из_пакета_takeoff_по_флагу():
    ...
    cfg = настройки()
    cfg["mission"]["manual_start"] = False
    # Режим задаётся явно: с 01.08.2026 умолчание конфига — змейка, а этот тест про
    # контур трассы. В режиме survey габариты из пакета применяются БЕЗ флага
    # (survey.plan_from_config), и тест проходил бы, ничего не проверяя.
    cfg["route"]["mode"] = "loop"
    cfg["loop"]["length"] = 6.0
    cfg["loop"]["width"] = 6.0
...
def test_пакет_не_меняет_трассу_по_умолчанию():
    ...
    ⚠️ Защита эта СУЩЕСТВУЕТ ТОЛЬКО У КОНТУРА. У змейки габариты из пакета
    применяются безусловно, и это решение осознанное (tests/test_survey.py ->
    test_размеры_из_пакета_takeoff_важнее_конфига: размеры зоны — параметр попытки).
    Но с 01.08.2026 змейка стала умолчанием, так что асимметрия теперь боевая:
    если автомобиль начнёт слать zone_w/zone_h, наша зона поиска поедет прямо
    в полёте. Пока трек автомобиля их не шлёт — поле остаётся пустым и конфиг цел.
    """
    from protocol import MessageFactory
