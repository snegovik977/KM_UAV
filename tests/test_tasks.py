...
Пункт 1 плана ещё не проверен полётом, и всё, что добавляется под пункт 2, обязано
проходить мимо него.
"""
...
def test_по_умолчанию_подзадача_231():
    """Пока пункт 1 не проверен полётом, поведение без ключей меняться не должно."""
    cfg = config_module.load()
    профиль = tasks_module.from_config(cfg)
    assert профиль.name == "2.3.1"
