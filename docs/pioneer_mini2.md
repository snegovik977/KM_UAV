...
2. Подключиться к Wi-Fi точке дрона:
   ...
   - **Пароль**: `geoscan123`
3. IP дрона: **`10.42.0.1`**

### Порты / сервисы

| URL | Что это |
|---|---|
| `http://10.42.0.1:9090/` | **Pioneer Code** — главный хаб, отсюда всё остальное |
| `http://10.42.0.1:9999/` | **Code OSS** — VS Code в браузере, пишем и запускаем Python прямо на борту |
| `http://10.42.0.1:7777/` | **AI Models** — реестр моделей, загрузка `.rknn` |
| `http://10.42.0.1:8089/<stream-name>` | **Видеопоток** от `ImageViewer` (сюда смотрим результат CV) |
...
frame = camera.get_cv_frame(timeout=5.0)   # numpy-массив, готов для OpenCV
camera.stop()
...
`http://10.42.0.1:8089/<name>`. Это наш способ смотреть результат детекции.
...
Через сервис **AI Models** (`http://10.42.0.1:7777/`) или меню
...
> **Пайплайн для нашего проекта**: обучаем YOLOv8 на своих данных → конвертируем в
...
`http://10.42.0.1:9999/` (или карточка «Пиши код как профи» в Pioneer Code).
