...
class VisionNode:
    ...
    def __init__(self, detector=None, model_name=MODEL_NAME, out_dir=OUT_DIR,
                 ...
                 use_roi=False, stride=1, record=True):
        ...
        self._stride = stride                 # тяжёлую обработку — раз в N кадров (производительность)
        self._record = record                 # False -> запись видео ведёт вызывающий (файл полёта)
    ...
    def _ensure(self, frame):
        ...
        try:
            ...
            self._rt = VisionRuntime(detector, out_dir=self._out_dir, frame_size=size,
                                     ...
                                     use_roi=self._use_roi, stride=self._stride,
                                     record=self._record)
...
def run_standalone(duration=None, require_aruco=True, tilt_camera=True, use_roi=False, stride=1):
    """Проверка без взлёта: камера -> детект -> трансляция квадратов. Ctrl+C для выхода.

    use_roi=False (по умолч.) — детект на КАЖДОМ кадре без фильтра «есть ли бассейн».
    Нужно для наземного теста (лодка на полу/ковре), где воды в кадре нет. --roi включит
    фильтр обратно (боевой пул: подавляет ложные вне воды).
    """
    ...
    node = VisionNode(require_aruco=require_aruco, use_roi=use_roi, stride=stride)
...
if __name__ == "__main__":
    ...
    ap.add_argument("--roi", action="store_true",
                    help="ВКЛючить фильтр бассейна (по умолч. ВЫКЛ: детект на каждом кадре)")
    ...
    ap.add_argument("--stride", type=int, default=1, help="тяжёлую обработку раз в N кадров (1=каждый, как v1.0)")
    a = ap.parse_args()
    run_standalone(duration=a.seconds, tilt_camera=not a.no_tilt, use_roi=a.roi,
                   stride=a.stride)
