...
class ShipMonitor:
    ...
    _msglog: list = field(default_factory=list)        # события для оверлея на трансляции (EN)
    ...
    def process_frame(self, frame: np.ndarray, t: float = 0.0):
        ...
        self._draw_msg_panel(annotated)          # события «терминала» колонкой справа (EN)
    ...
    def _count_and_announce(self, tid, track, d, t):
        ...
        if key not in self.seen:
            ...
            self._msglog.append(f"detected {cls}")         # оверлей на трансляции (EN)
        ...
        if track.aruco_id is not None and rec.get("aruco_id") is None:
            ...
            if track.aruco_id not in self._announced_ids:
                ...
                self._msglog.append(f"  -> id {track.aruco_id}")
    ...
    def _draw_msg_panel(self, img, max_lines=12):
        """Лог событий колонкой в правой части кадра (полупрозрачно, чтобы не закрывать).

        Английский: cv2.putText не рисует кириллицу. В терминал идёт русский по регламенту,
        это — дублирование событий на самой трансляции.
        """
        if not self._msglog:
            return
        H, W = img.shape[:2]
        msgs = self._msglog[-max_lines:]
        lh, pad = 22, 8
        panel_w = min(300, W // 3)
        x0, y0 = W - panel_w, 6
        y1 = min(H, y0 + pad + lh * (len(msgs) + 1) + pad)
        roi = img[y0:y1, x0:W]                    # затемняем ТОЛЬКО зону панели
        if roi.size:
            cv2.addWeighted(np.full_like(roi, 25), 0.5, roi, 0.5, 0, roi)
        y = y0 + pad + 14
        self._put(img, "EVENTS", x0 + pad, y, (0, 200, 255))
        for m in msgs:
            y += lh
            self._put(img, m, x0 + pad, y, (255, 255, 255))

    @staticmethod
    def _put(img, text, x, y, color):
    ...
    def summary(self):
        ...
        # ships — подробный список уникальных судов с id (для детального вывода итогов)
        ships = [{"class": r["class"], "aruco_id": r.get("aruco_id")} for r in uniques]
        return {"total": reg + unreg, "registered": reg, "unregistered": unreg,
                "ships": ships, "log": self.log}
