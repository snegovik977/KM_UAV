...
CLASS_RU_HEAD = {"registered": "Зарегистрированные суда",
                 "unregistered": "Незарегистрированные суда"}
...
class VisionRuntime:
    def __init__(self, detector, *,
                 ...
                 use_roi=False, require_aruco=True, stride=1, record=True):
        """
        ...
        # record=False -> запись ведёт вызывающий (файл полёта), тут не дублируем.
    ...
    def process(self, frame, t: float = 0.0):
        ...
        # События РАЗДЕЛЕНЫ: "class" — классификация (+20, не ждёт ArUco), "id" — метка (+5).
        # Ведущий '\n' и flush — чтобы сообщение не «влипло» в строку таймера обратного
        # отсчёта (тот пишет через '\r' из другого потока). Форматы строго по регламенту.
        for kind, d, rec in events:
            if kind == "class":
                sys.stdout.write(f"\nОбнаружено {CLASS_RU[rec['class']]} судно\n")
                sys.stdout.flush()
                self._save_photofix(frame, d, rec)         # фотофиксация при обнаружении
            elif kind == "id":
                sys.stdout.write(f"\nОбнаружено судно с id {rec['aruco_id']}\n")
                sys.stdout.flush()

        # запись кадра в видео (если ведём её тут)
        if self.writer is not None:
            out = annotated
            if (out.shape[1], out.shape[0]) != self._frame_size:
                out = cv2.resize(out, self._frame_size)
    ...
    def finish(self):
        ...
        # Блок итогов: сводка (форматы регламента) + ПОДРОБНЫЙ список судов с id (запрос оргов).
        lines = ["\n=== ИТОГИ ===",
        ...
        for cls_key in ("registered", "unregistered"):
            ships = [sh for sh in s.get("ships", []) if sh["class"] == cls_key]
            if not ships:
                continue
            lines.append(f"\n{CLASS_RU_HEAD[cls_key]} ({len(ships)}):")
            for sh in ships:
                aid = sh.get("aruco_id")
                lines.append(f"  - {'id ' + str(aid) if aid is not None else 'без id'}")
        sys.stdout.write("\n".join(lines) + "\n")
        sys.stdout.flush()
        ...
        if self.writer is not None:
            ...
            print(f"[vision] лог: {self.log_path} (видео пишет файл полёта)")
        return s
