...
# Записи попыток и журнал хаба — одной папкой (config.yaml -> mission.record_dir,
# hub.py --log). В корне они тонули: за день набегает два десятка пар mp4+jsonl.
ЗАПИСИ = os.path.join(КОРЕНЬ, "records")
...
def забрать_записи(хост=None, пользователь=None, рабдир=РАБДИР):
    ...
    Кладём в ЗАПИСИ, а не в корень: за день набегает два десятка пар mp4+jsonl
    (`mission.record_dir` в config.yaml — та же папка и на борту).
    """
    if not os.path.isdir(ЗАПИСИ):
        os.makedirs(ЗАПИСИ)
    цель = "%s:%s/records/flight_*" % (адрес_борта(хост, пользователь), рабдир)
    скажи("[пульт] забираю записи полёта -> %s" % os.path.basename(ЗАПИСИ))
    subprocess.call(["scp"] + ssh_опции() + [цель, ЗАПИСИ])
...
def пульт(процессы, url, аргументы, борт):
    ...
    if борт and спроси_очередью("Забрать записи полёта с борта?", ввод, True, аргументы.авто):
        ...
        скажи("[пульт] разбор: %s tools/record.py records/flight_*.jsonl"
              % os.path.basename(PY))
    return 0
